...
Java code has a column limit of 100 characters. Except as noted below, any line that would exceed this limit must be line-wrapped, as explained in Section 4.5, [Line-wrapping](#45-line-wrapping).
...
1. When a line is broken at a _non-assignment_ operator the break comes before the symbol. (Note that this is not the same practice used in Google style for other languages, such as C++ and JavaScript.)
...
When there are multiple _continuation lines_, indentation may be varied beyond +4 as desired. In general, two continuation lines use the same indentation level if and only if they begin with syntactically parallel elements.
...
This practice is permitted, but is **never required** by Java Code Style. It is not even required to maintain horizontal alignment in places where it was already used.
...
**Terminology Note**: Inside the braces of a _switch block_ are one or more _statement groups_. Each statement group consists of one or more switch labels (either case `FOO:` or `default:`), followed by one or more statements (or, for the last statement group, zero or more statements).
...
**Exception**: A switch statement for an `enum` type _may_ omit the `default` statement group, if it includes explicit cases covering all possible values of that type. This enables IDEs or other static analysis tools to issue a warning if any cases were missed.
...
In Java Code Style special prefixes or suffixes, like those seen in the examples `name_`, `mName`, `s_name` and `kName`, are not used.
...
static final ImmutableMap<String, Integer> AGES =
        ImmutableMap.of("Ed", 35, "Ann", 32);
...
### 6.5. Let your callers construct support objects
```java
// Bad.
...
#### 6.6.2. Avoid randomness in tests
...
### 6.8. Remove dead code
Delete unused code (imports, fields, parameters, methods, classes) including commented out source code. They will only rot.

### 6.9. Use general types
...
### 6.11. When interrupted, reset thread interrupted state
...
### 6.12. Avoid unnecessary code
Dot use superfluous temporary variables:
```java
// Bad.
...
It's recommended that you check the official Oracle [documentation](http://www.oracle.com/technetwork/articles/java/index-137868.html) about how to write Javadoc. The guidelines in this section inherit Oracle's recommendation and override some of them.
...
Additional Javadoc content may also be present, as explained in Section 7.3.4, [Non-required Javadoc](#731-exception-self-explanatory-methods).
...
Code can change hands numerous times in its lifetime, and quite often the original author of a source file is irrelevant after several iterations.

### 7.5. Package

Each package of the public API must have a `package-info.java` file composed according to the following format:
```java
/*
 * <COPYRIGHT_STATEMENT>
 */
...
/**
 * <DESCRIPTION_OF_THE_PACKAGE>
 */
package <PACKAGE_NAME>;
```

As for the `<COPYRIGHT_STATEMENT>` see the [copyright_header.md](copyright_header.md) file.
